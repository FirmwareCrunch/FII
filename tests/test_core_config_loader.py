...
from FII.core.emulator_types import MemoryRange
from FII.core.config_loader import (
...
def test_config_loader_load_config_invalid_file_ext():
    with pytest.raises(ConfigLoaderError):
        load_yaml_config({}, "", '.txt')


def test_config_loader_load_config_yaml_file():
...
def test_config_loader_load_config_json_file():
    schema = {'type': 'dict', 'schema': {
                'test_conf': {'type': 'dict',
...
def test_config_loader_load_config_yaml_file_invalid_config():
    schema = {'type': 'dict', 'schema': {
                'test_conf': {'type': 'dict',
                              'schema': {'address': {'type': 'integer'}}}}}
    yaml_conf = """
...
def test_config_validator_check_with_hex_int64_value_interger():
    schema = {'type': 'dict', 'schema': {
