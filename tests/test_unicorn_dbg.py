...
class TestFrontendBreakpointSet:
    ...
    def test(self, caplog):
        caplog.set_level(logging.INFO, 'FII.unicorn_dbg')
        self.count = 0
        emu_dbg = BinBlob2Dbg(BlobArmEl32IncLoop,
                              debug_event_callback=self.callback)
        front = UnicornDbgFrontend(emu_dbg.dbg, EmulatorShell())
        ...
        with patch("FII.plugins.emulator_shell.EmulatorShell"
...
class TestFrontendDbgSetStepFromBp:
    ...
    def test(self, caplog):
        ...
        with patch("FII.plugins.emulator_shell.EmulatorShell"
                   ".resume_user_interact"), \
                patch("FII.plugins.emulator_shell.EmulatorShell"
                      ".resume_emu_exec"), \
                patch("FII.plugins.emulator_shell.EmulatorShell"
