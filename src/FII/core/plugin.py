...
class PluginManager:
    def __init__(self):
        self.logger = logging.getLogger('FII.plugin_loader')
        self.plugins_context = FiiPluginContext()
        self._plugins_store: Dict[str, PluginStoreEntry] = {}

    @staticmethod
    def is_emulator_plugin(obj: Any):
        return (
            True if inspect.isclass(obj) and inherits_from(obj, FiiPlugin)
            else False)

    @classmethod
    def _find_builtin_plugins(cls) -> List[Type[FiiPlugin]]:
        ...
        return cast(List[Type[FiiPlugin]], plugins)

    @classmethod
    def _find_extra_plugins(cls, path: str) -> List[Type[FiiPlugin]]:
        plugins = pkg_object_loader(path, cls.is_emulator_plugin)
        return cast(List[Type[FiiPlugin]], plugins)

    @staticmethod
    def _sort_plugin_by_priority(
        plugins: List[Type[FiiPlugin]]
    ) -> Dict[int, List[Type[FiiPlugin]]]:
        plugin_by_priority: Dict[int, List[Type[FiiPlugin]]] = {}
        for plug in plugins:
    ...
    def _get_plugin_requirements(
        plugin: Type[FiiPlugin], init_context: FiiPluginContext
    ) -> Dict[str, Any]:
        requirements = {}
